...
Expr *expr_clone(const Expr *e) {
    if (!e) return NULL;
    switch (e->tag) {
        case E_STAR:  return e_star();
        case E_BOX:   return e_box();
        case E_TRUE:  return e_true();
        case E_FALSE: return e_false();
        case E_BOOL:  return e_bool();
        case E_VAR:   return e_var(e->name);
        case E_LAM:   return e_lam(e->binder, expr_clone(e->type), expr_clone(e->body));
        case E_PI:    return e_pi (e->binder, expr_clone(e->type), expr_clone(e->body));
        case E_APP:   return e_app(expr_clone(e->fun), expr_clone(e->arg));
        case E_IF:    return e_if(expr_clone(e->cond),
                                  expr_clone(e->then_br),
...
bool expr_free_in(const char *x, const Expr *e) {
    ...
    switch (e->tag) {
        ...
        case E_LAM: case E_PI:
            if (expr_free_in(x, e->type)) return true;
            if (strcmp(x, e->binder) == 0) return false; /* shadowed */
            return expr_free_in(x, e->body);
        case E_APP:
            return expr_free_in(x, e->fun) || expr_free_in(x, e->arg);
        case E_IF:
            return expr_free_in(x, e->cond) ||
                   expr_free_in(x, e->then_br) ||
...
/*
 * Precedence:
 *   0 — binders / if  (rightmost, lowest)
 *   1 — application   (left-associative)
 *   2 — atom
 */

static void print_parens_open (int need) { if (need) putchar('('); }
static void print_parens_close(int need) { if (need) putchar(')'); }

void expr_print(const Expr *e, int prec) {
    if (!e) { printf("<null>"); return; }
    switch (e->tag) {
        case E_STAR:  printf("\xe2\x98\x85"); return;   /* ★ */
        case E_BOX:   printf("\xe2\x96\xa1"); return;   /* □ */
        case E_TRUE:  printf("true");  return;
        case E_FALSE: printf("false"); return;
        case E_BOOL:  printf("Bool");  return;
        case E_VAR:   printf("%s", e->name); return;
        ...
        case E_PI: {
            int need = (prec > 0);
            print_parens_open(need);
            /* If the binder is not free in the body, print as A -> B */
            if (!expr_free_in(e->binder, e->body)) {
                expr_print(e->type, 1); /* left of -> needs prec 1 */
                printf(" \xe2\x86\x92 ");               /* → */
                expr_print(e->body, 0);
            } else {
