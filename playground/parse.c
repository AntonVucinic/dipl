 ...
 *   app ('->' expr)?                  -- non-dependent pi sugar  A -> B
 *   app                               --   (plain application)
 *
 * Note: '->' is right-associative and lower precedence than application,
 * so  A -> B -> C  parses as  A -> (B -> C),  and
 *     f x -> B     parses as  (f x) -> B.
 */
static Expr *parse_expr(Lexer *lx) {
    ...
    if (pk.kind == TOK_LAMBDA) {
        ...
        if (var.kind != TOK_IDENT) {
            fprintf(stderr,
                    "Parse error: expected variable after λ, got '%s'\n",
                    var.text);
            exit(1);
        }
        expect(lx, TOK_COLON, "':'");
    ...
    if (pk.kind == TOK_PI) {
        ...
        if (var.kind != TOK_IDENT) {
            fprintf(stderr,
                    "Parse error: expected variable after Π, got '%s'\n",
                    var.text);
            exit(1);
        }
        expect(lx, TOK_COLON, "':'");
    ...
    /* app ( -> expr )*   — right-associative non-dependent Pi */
...
static Expr *parse_app(Lexer *lx) {
    ...
    while (1) {
        Token pk = lexer_peek(lx);
        /* atom starters */
        if (pk.kind != TOK_IDENT  && pk.kind != TOK_TRUE  &&
            pk.kind != TOK_FALSE  && pk.kind != TOK_BOOL  &&
            pk.kind != TOK_STAR   && pk.kind != TOK_BOX   &&
            pk.kind != TOK_LPAREN)
            break;
...
static Expr *parse_atom(Lexer *lx) {
    Token pk = lexer_peek(lx);
    switch (pk.kind) {
        case TOK_IDENT:  lexer_consume(lx); return e_var(pk.text);
        case TOK_STAR:   lexer_consume(lx); return e_star();
        case TOK_BOX:    lexer_consume(lx); return e_box();
        case TOK_BOOL:   lexer_consume(lx); return e_bool();
        case TOK_TRUE:   lexer_consume(lx); return e_true();
        case TOK_FALSE:  lexer_consume(lx); return e_false();
        case TOK_LPAREN: {
            lexer_consume(lx);
...
Expr *parse(const char *src) {
    ...
    if (eof.kind != TOK_EOF) {
        fprintf(stderr,
                "Parse error: trailing tokens starting with '%s'\n",
                eof.text);
        exit(1);
    }
    return e;
