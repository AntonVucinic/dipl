...
/*
 * fresh(hint, avoid_s, avoid_e)
 *
 * Returns a new name based on `hint` that is not free in either
 * `avoid_s` (the substituted term) or `avoid_e` (the body being
 * substituted into).  We simply append a numeric suffix and increment
 * until we find a name that works.
 */
static char fresh_buf[128];

static const char *fresh(const char *hint,
                          const Expr *avoid_s,
                          const Expr *avoid_e) {
    int n = 0;
    while (1) {
        ...
        if (!expr_free_in(fresh_buf, avoid_s) &&
            !expr_free_in(fresh_buf, avoid_e))
            return fresh_buf;
        n++;
    }
}
    ...
    switch (e->tag) {
        ...
        case E_PI: {
            ...
            if (strcmp(e->binder, x) == 0) {
                ...
                return (e->tag == E_LAM)
                    ? e_lam(e->binder, new_ty, new_body)
...
Expr *whnf(const Expr *e) {
    ...
    switch (e->tag) {
        ...
        case E_VAR:  case E_LAM:  case E_PI:
            return expr_clone(e);
        ...
        case E_IF: {
            ...
            if (cond->tag == E_TRUE)  return whnf(e->then_br);
            if (cond->tag == E_FALSE) return whnf(e->else_br);
...
/* ------------------------------------------------------------------ */
/* Full normal form (reduce everywhere, including under binders)        */
/* ------------------------------------------------------------------ */
...
Expr *nf(const Expr *e) {
    ...
    switch (e->tag) {
        ...
        case E_IF: {
            Expr *cond = nf(e->cond);
            if (cond->tag == E_TRUE)  return nf(e->then_br);
            if (cond->tag == E_FALSE) return nf(e->else_br);
