...
CubeVertex current_vertex = CUBE_LAMBDA_P; /* default: λP */
...
/*
 * rule(s1, s2) — is the pair (s1,s2) allowed at the current vertex?
 *
 * This is the *only* place that changes as we walk the cube.
 * s1 is the sort of the domain A,  s2 is the sort of the codomain B,
 * in a Pi type  Πx:A.B.
 *
 * PTS presentation:
 *   Axiom:   ★ : □
 *   Rule:    if (s1,s2) in rules  then  Πx:A.B : s2
 *            when  A:s1  and  B:s2
 */
typedef struct { ExprTag s1; ExprTag s2; } SortPair;

static int sort_pair_allowed(ExprTag s1, ExprTag s2) {
    /* (★,★) is always allowed */
    if (s1 == E_STAR && s2 == E_STAR) return 1;

    switch (current_vertex) {
        case CUBE_LAMBDA_ARROW:
            return 0;

        case CUBE_LAMBDA_P:
            /* (★,□): types may depend on terms */
            return (s1 == E_STAR && s2 == E_BOX);

        case CUBE_LAMBDA_2:
            /* (□,★): terms may depend on types (polymorphism) */
            return (s1 == E_BOX && s2 == E_STAR);

        case CUBE_LAMBDA_OMEGA:
            /* (□,□): type operators */
            return (s1 == E_BOX && s2 == E_BOX);

        case CUBE_LAMBDA_P2:
            return (s1 == E_STAR && s2 == E_BOX) ||
                   (s1 == E_BOX  && s2 == E_STAR);

        case CUBE_LAMBDA_P_OMEGA:
            return (s1 == E_STAR && s2 == E_BOX) ||
                   (s1 == E_BOX  && s2 == E_BOX);
...
Expr *env_lookup(Env *env, const char *name) {
    for (Env *e = env; e; e = e->rest)
        if (!strcmp(e->name, name)) return e->type;
    return NULL;
}
 ...
 * conv(a, b, env)
 *
 * We reduce both sides to whnf, then:
 *   - sorts and constants are compared by tag
 *   - variables by name
 *   - App: compare fun and arg recursively
 *   - Lam/Pi: compare types, then bodies (with the binder in scope)
 *             we alpha-rename b's binder to a's binder via substitution
 */
int conv(const Expr *a, const Expr *b, Env *env) {
    Expr *a2 = whnf(a);
    Expr *b2 = whnf(b);

    if (a2->tag != b2->tag) return 0;
    ...
    switch (a2->tag) {
        ...
        case E_APP:
            return conv(a2->fun, b2->fun, env) &&
                   conv(a2->arg, b2->arg, env);

        case E_LAM:
        case E_PI: {
            if (!conv(a2->type, b2->type, env)) return 0;
...
static void type_error(const char *msg, const Expr *e) {
    ...
    if (e) { fprintf(stderr, "  in: "); expr_println(e); }
    exit(1);
}

/* Return the tag of the whnf of e, or -1 if not a sort */
...
Expr *typecheck(Expr *e, Env *env) {
    switch (e->tag) {
        ...
        /* Primitives */
        case E_TRUE:  return e_bool();
        case E_FALSE: return e_bool();
        case E_BOOL:  return e_star();   /* Bool : ★ */
         ...
         *   Γ ⊢ λx:A.e : Πx:A.B
         *
         * We synthesize the Pi type and verify it is well-sorted,
         * effectively reusing the Pi rule for the well-formedness check.
         */
        case E_LAM: {
            /* Check that A is a valid classifier (a type or kind) */
            ...
            /* Verify the resulting Pi type is well-sorted */
        ...
        case E_APP: {
            ...
            if (fty_whnf->tag != E_PI) {
                ...
                fprintf(stderr, "  function: "); expr_println(e->fun);
                fprintf(stderr, "  its type: "); expr_println(fty);
                exit(1);
            }
            ...
            if (!conv(fty_whnf->type, aty, env)) {
                ...
                fprintf(stderr, "  expected: "); expr_println(fty_whnf->type);
                fprintf(stderr, "  got:      "); expr_println(aty);
                exit(1);
            }
